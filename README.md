...
•	Difficulty in identifying churn-prone customers before they leave

•	Lack of insight into why customers churn

•	High marketing and retention costs due to poor targeting

•	Loss of revenue, customer lifetime value, and market share

•	Manual decision-making with limited predictive intelligence
...
•	Target Retention Rate: Even a modest improvement in retention can have a huge impact. For example, increasing retention rates by just 5% can boost profits by 25% to 95% (Bain & Company).

•	Targeted Intervention: By identifying customers with a high probability of churn AND a high CLV (Phase 5: Revenue Impact Analysis), the bank can target retention efforts with maximum efficiency.
...
•	Reduced Retention Waste: Instead of offering expensive incentives to all customers or those not actually at risk, the model directs campaigns to the Top 5%-10% most likely to churn. This can reduce unnecessary spending by 50% or more.

•	Increased Campaign ROI: By understanding the key factors contributing to churn (Objective ii), the bank can tailor retention offers (e.g., "low-interest credit card" vs. "fee waiver") to the specific reason the customer is leaving, significantly increasing the campaign's success rate.

•	Efficiency in Customer Service: Proactive outreach can resolve issues before they escalate, reducing the need for costly, reactive complaint resolution later in the customer life cycle.
...
•	Matplotlib, Seaborn, and Plotly for insights and visual analytics
...
•	Open your terminal and create a fresh new database file called customer_analytics.db. Run the command
```
...
print("✅ churn_label created and saved to bank_customers table.")
...
for col in drop_cols:
    if col not in df.columns:
        print(f"⚠️ Warning: '{col}' not found in dataframe. Skipping drop for this column.")
...
# 6. Train-test split
# -----------------------------
print("✂️ Splitting into train and test sets (80/20, stratified)...")
...
print(f"💾 Saving preprocessing pipeline to: {pipeline_path}")
...
# ----------------------------------------
print("\n🗄️ Preparing database for probability storage...")
DB_PATH = "../database/customer_analytics.db"
conn = sqlite3.connect(DB_PATH)
cursor = conn.cursor()

# Add columns only if missing
print("🔍 Updating bank_customers structure...")
...
if "churn_probability" not in existing_columns:
    ...
    print("✔ Added churn_probability column")
...
print("✔ revenue_insights table ready for inserts")
...
print("📥 Loading bank_customers table...")
df = pd.read_sql_query("SELECT * FROM bank_customers", conn)

if "churn_label" not in df.columns:
    raise ValueError("❌ churn_label not found. Ensure Step 1 & 2 are completed correctly.")
...
print("⚠️ Risk Tier Distribution:")
print(df["risk_tier"].value_counts())

# ----------------------------------------
# Update customer table with predictions
# ----------------------------------------
print("💾 Updating bank_customers with predictions...")
df.to_sql("bank_customers", conn, if_exists="replace", index=False)

# ----------------------------------------
# Update revenue_insights table too
# ----------------------------------------
print("💾 Updating revenue_insights...")
...
print("📥 Loading best churn prediction model...")
...
print("\n📊 Phase 3 — Step 2: Performance Evaluation Metrics")
...
print("\n📈 Phase 3 — Step 3: ROC Curve & Confusion Matrix")
...
print("\n📊 Phase 3 — Step 4: Feature Importance Analysis")
...
print("\n🔍 Phase 3 — Step 5: SHAP Explainability")
...
if isinstance(raw_shap_values, list):
    ...
    print("🟢 Using shap_values for binary classification")
else:
    shap_values = raw_shap_values
    print("🟢 Using shap_values matrix directly")
...
print("\n🔝 Top 10 Customers With Highest Revenue Exposure (₦):")
...
print("\n💰 Total Revenue at Risk: ₦{:,.2f}".format(total_risk))
print("\n📌 Revenue Risk Contribution by Tier:")
...
def recommend(row):
    if row["revenue_risk"] > 400000:
        return "🧲 Personalized VIP retention package + Dedicated RM"
    elif row["churn_probability"] > 0.85:
        return "📞 Urgent outreach: Complaint resolution & product review"
    elif row["estimated_clv"] > 350000:
        return "💳 Offer premium product upgrade / loan restructuring"
    else:
        return "📍 Maintain digital engagement & loyalty incentives"

vip_critical["recommended_strategy"] = vip_critical.apply(recommend, axis=1)

print("\n🔥 VIP Critical Retention Actions Needed:")
...
iv.	We segmented customers into actionable tiers — ensuring high-value customers receive priority intervention
